(function(){
  ...
  const icons = {
    ...
    settings:'<svg viewBox="0 0 24 24"><path d="M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8zM4 12H2M22 12h-2M12 4V2M12 22v-2M5.6 5.6l1.4 1.4M17 17l1.4 1.4M18.4 5.6 17 7M7 17l-1.4 1.4"/></svg>'
  };
  const groups = [
  ...
  function run(){
    ...
    const html = `<aside class="v7-sidebar clean-svg-nav-sidebar">
      ...
      <nav>${groups.map(([title, links]) => `${title ? `<div class="v7-nav-title">${title}</div>` : ''}<div class="v7-nav-list">${links.map(([href,key,label]) => `<a class="v7-nav-link ${activeFor(href) ? 'active' : ''}" href="${href}"><span class="v7-nav-icon">${icons[key] || ''}</span><span>${label}</span></a>`).join('')}</div>`).join('')}</nav>
    </aside>`;
