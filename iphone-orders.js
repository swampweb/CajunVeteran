console.log('iphone-orders.js v1.3 stable card collapse fix loaded');
...
window.toggleMobileCard=function(id,event,source='card'){
  ...
  const details=card.querySelector('.mobile-card-expanded');
  if(details){
    details.hidden=!willExpand;
    details.style.setProperty('display', willExpand ? 'block' : 'none', 'important');
  }

  const btn=card.querySelector('.mobile-expand-btn');
...
function card(o){
  ...
  return `<article class="mobile-card collapsed-mobile-card ${paid?'paid':''} ${posted?'posted':''}" data-mobile-id="${o.id}" onclick="toggleMobileCard(${o.id}, event)">
    ...
    <div class="mobile-card-expanded" hidden style="display:none!important">
    ...
    <button type="button" class="mobile-expand-btn" onclick="toggleMobileCard(${o.id}, event, 'button')">Expand Card</button>
  </article>`;
}
window.toggleMobileWeekGroup=function(button,event){
  ...
  if(!group) return;
  const body=group.querySelector('.mobile-week-body');
