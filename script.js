...
// ---------- PUBLIC ----------
function ordersStatusSelectHtml(order){
  return `<select data-act="pvStatus" data-id="${order.orderId}" style="background:#0a1827;color:#e9edf2;border:1px solid #28415e;border-radius:4px;padding:3px 6px">
`+
    `<option value="new" ${order.status==='new'?'selected':''}>New</option>`+
    `<option value="in_progress" ${order.status==='in_progress'?'selected':''}>In Process</option>`+
    `<option value="completed" ${order.status==='completed'?'selected':''}>Completed</option>`+
    `<option value="shipped" ${order.status==='shipped'?'selected':''}>Shipped/Delivered</option>`+
  `</select>`;
}
function ordersPaidToggleHtml(order){
  return `${order.paid?`<span class="badge ok">Paid</span>`:`<span class="badge muted">Unpaid</span>`} `+
         `<label class="paid-toggle" style="margin-left:6px;cursor:pointer;"><input type="checkbox" data-act="togglePaid" data-id="${order.orderId}" ${order.paid?'checked':''}/> paid</label>`;
}
...
if (typeof document !== 'undefined'){
  document.addEventListener('change', function(e){
    ...
    if(t.getAttribute('data-act')==='togglePaid'){
      var id=t.getAttribute('data-id'); var s=loadStore(); var idx=s.orders.findIndex(o=>String(o.orderId)==String(id)); if(idx>=0){ s.orders[idx].paid=!!t.checked; saveStore(s); renderOrdersPublic?.(); renderAdminTables?.(); }
    }
    if(t.getAttribute('data-act')==='pvStatus'){
      var id2=t.getAttribute('data-id'); var s2=loadStore(); var j=s2.orders.findIndex(o=>String(o.orderId)==String(id2)); if(j>=0){ s2.orders[j].status=t.value; saveStore(s2); renderOrdersPublic?.(); renderAdminTables?.(); }
    }
  });
}
...
function setupForms(){
  ...
  function updateLinePrice(lineContainer, sku){ try { const s=loadStore(); const it=(s.items||[]).find(i=>i.sku===sku); const el=lineContainer.querySelector('.line-price'); if(!el) return; if(it && it.price!=='' && it.price!==undefined && !isNaN(Number(it.price))) el.textContent = `Price: $${Number(it.price).toFixed(2)}`; else el.textContent = 'Price: —'; } catch {}
  }
...
function setupBackup(){ const dl=document.getElementById('downloadBackup'); if(dl){ dl.addEventListener('click',()=>{ try{ const raw=localStorage.getItem(STORAGE_KEY)||JSON.stringify({items:[],orders:[],colors:[]}); const blob=new Blob([raw],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='inventory_orders_backup.json'; a.click(); }catch(e){ alert('Could not create backup: '+e.message); } }); }
  const restore=document.getElementById('restoreFile'); if(restore){ restore.addEventListener('change',(e)=>{ const file=e.target.files&&e.target.files[0]; if(!file) return; const reader=new FileReader(); reader.onload=()=>{ try{ let text=reader.result; if(typeof text==='string' && text.charCodeAt(0)===0xFEFF){ text=text.slice(1); } let data=JSON.parse(text);
      if(Array.isArray(data)){ data={items:[],orders:data,colors:[]}; }
      if(!data || (!Array.isArray(data.items) && !Array.isArray(data.orders))){ if(data && data.store){ data=data.store; } }
      if(!data || !Array.isArray(data.items) || !Array.isArray(data.orders)){ throw new Error('Invalid backup. Expected keys: items[], orders[], optional colors[].'); }
      if(!Array.isArray(data.colors)) data.colors=[];
      data.orders=data.orders.map(o=>({ orderId:String(o.orderId||''), customer:String(o.customer||''), lines:Array.isArray(o.lines)? o.lines.map(l=>({ sku:String(l.sku||''), qty:Number(l.qty||1)||1, colors:Array.isArray(l.colors)? l.colors:[] })) : [], orderDate:o.orderDate||'', dueDate:o.dueDate||'', status:o.status||'new', priority:Number(o.priority||3)||3, discountType:o.discountType||'none', discountValue:Number(o.discountValue||0)||0, surchargeType:o.surchargeType||'none', surchargeValue:Number(o.surchargeValue||0)||0, notes:o.notes||'', paid:!!o.paid }));
      data.items=data.items.map(i=>({ sku:String(i.sku||''), name:String(i.name||''), size:i.size||'', price:(i.price===''||i.price===undefined)?'':Number(i.price), costMaterial:(i.costMaterial===''||i.costMaterial===undefined)?'':Number(i.costMaterial), qty:Number(i.qty||0)||0, desc:i.desc||'', length:(i.length===''||i.length===undefined)?'':Number(i.length), width:(i.width===''||i.width===undefined)?'':Number(i.width), height:(i.height===''||i.height===undefined)?'':Number(i.height), image:i.image||'', colors:Array.isArray(i.colors)? i.colors:[], visible:(i.visible===false)?false:true }));
      saveStore(data); alert('Backup restored successfully.'); renderAdminTables(); refreshColorSelects(); refreshOrderItemSelects(); suggestNextOrderId(); suggestNextSku();
    }catch(err){ alert('Restore failed: '+err.message); console.error(err); } finally { restore.value=''; } };
    reader.onerror=()=>{ alert('Could not read the selected file.'); restore.value=''; };
